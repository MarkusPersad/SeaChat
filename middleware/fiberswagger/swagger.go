...
func New(config ...Config) fiber.Handler {
	...
	if len(rawSpec) == 0 {
		...
		if err != nil {
			log.Logger.Fatal().Err(err).Msgf("Failed to read provided Swagger file (%s): %v", cfg.FilePath, err.Error())
		}
	}
	...
	if errJSON != nil && errYAML != nil {
		log.Logger.Fatal().Errs("Unmarshal Error",  []error{errJSON,errYAML}).Msgf("Failed to parse the Swagger spec as JSON or YAML: JSON error: %s, YAML error: %s", errJSON, errYAML)
